...
import { infraImages } from "@/config/clinic";
  ...
  return (
    <section id="studio" className="scroll-mt-24 bg-background px-4 py-16 sm:px-8 lg:px-16 lg:py-24">
      <div className="mx-auto w-full max-w-[1440px]">
        <SectionHeader badge={t.studio.badge} title={t.studio.title} desc={t.studio.desc} />

        <div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {infraImages.map((src, index) => {
            const alt = t.studio.gallery[index] ?? "";
            return (
              <div
                key={src}
                className={cn(
              ...
              >
                ...
                <div className="absolute inset-x-0 bottom-0 bg-[linear-gradient(to_top,rgba(12,12,14,0.82),transparent)] p-4 pt-10">
                  <span className="font-heading text-[0.8125rem] font-bold text-white">{alt}</span>
                </div>
              </div>
            );
          })}
        ...
        <ul className="mt-8 grid gap-3 md:grid-cols-3">
          {t.studio.highlights.map((highlight) => (
            <li
              key={highlight}
